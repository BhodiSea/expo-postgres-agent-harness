...
1. **Transport security (ATS / cleartext)**: no `NSAllowsArbitraryLoads: true` and
   ...
   reviewed reason). SOURCE: https://docs.expo.dev/versions/latest/config/app/
...
5. **CNG purity**: `apps/mobile/android/**` and `apps/mobile/ios/**` are GENERATED
...
6. **SecureStore one-door**: credentials (access + refresh tokens) exist ONLY behind
...
7. **Secret hygiene**: no `EXPO_PUBLIC_`-prefixed secret-shaped names (KEY / SECRET /
...
8. **eas.json sanity**: `cli.appVersionSource` stays `"local"` and profiles keep
   `autoIncrement: false` (the repo is the version source of truth — a remote
   counter is a surface no gate can diff); the per-profile `node`/`pnpm` pins stay;
   no plaintext secret values in profile `env` blocks.
