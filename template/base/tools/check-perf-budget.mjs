...
for (const entry of budget.subjects) {
  if (!existsSync(resolve(process.cwd(), entry.subject))) {
    errs.push(
      `subjects[] declares "${entry.subject}" but the file does not exist — restore it or remove the entry; harness exemplars can be pulled with \`npx expo-postgres-agent-harness update --refresh-seeded <path>\``,
    )
  }
}
