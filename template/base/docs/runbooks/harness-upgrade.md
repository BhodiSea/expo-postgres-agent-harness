...
1. **Sweep.** Run `pnpm validate` and fix everything the ramped check reports in
   ...
   first (`npx expo-postgres-agent-harness update --refresh-seeded <path>` — the
...
2. **Bump `baseVersion`** in `.harness/manifest.json` to the version the NOTE
   ...
   plain editor is fine), or run `npx expo-postgres-agent-harness graduate` —
...
```
npx expo-postgres-agent-harness update --refresh-seeded tools/perf-budget.json
```
