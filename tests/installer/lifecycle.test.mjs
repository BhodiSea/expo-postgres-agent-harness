...
test('retrofit rejects Next.js, Tauri, foreign lockfiles, and non-workspace layouts with clear messages', () => {
  const nextDir = mkdtempSync(join(tmpdir(), 'epah-next-'))
  ...
  assert.ok(nextRes.out.includes('Expo (React Native) + Hono'), nextRes.out)

  const tauriDir = mkdtempSync(join(tmpdir(), 'epah-tauri-'))
  ...
  const npmDir = mkdtempSync(join(tmpdir(), 'epah-npmlock-'))
  writeFileSync(join(npmDir, 'package.json'), JSON.stringify({ dependencies: { hono: '^4.0.0' } }))
  ...
  const bareDir = mkdtempSync(join(tmpdir(), 'epah-bare-'))
