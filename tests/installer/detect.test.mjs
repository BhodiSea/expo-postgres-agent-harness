...
test('package.json with a `next` dependency throws with the redirect story', () => {
  const d = dir('epah-det-next-')
  writePkg(d, { name: 'x', dependencies: { next: '16.0.0' } })
  assert.throws(
    () => detect(d),
    (/** @type {Error} */ err) => {
      assert.match(err.message, /Expo \(React Native\) \+ Hono/)
...
test('`next` in devDependencies is rejected too (deps and devDeps are merged)', () => {
  const d = dir('epah-det-nextdev-')
  writePkg(d, { name: 'x', devDependencies: { next: '16.0.0' } })
  assert.throws(() => detect(d), /Expo \(React Native\) \+ Hono/)
})
...
test('pnpm workspace without expo or hono markers throws with layout guidance', () => {
  const d = dir('epah-det-nomark-')
  writePkg(d, { name: 'x' })
  writeFileSync(join(d, 'pnpm-workspace.yaml'), "packages:\n  - 'apps/*'\n")
  assert.throws(
    () => detect(d),
    (/** @type {Error} */ err) => {
      assert.ok(err.message.includes('apps/mobile Expo app'), err.message)
...
test('retrofit: app.config.ts marker alone → hasExpo true, hasHono false, pkg round-trips', () => {
  const d = dir('epah-det-expo-')
  ...
  assert.deepEqual(detect(d), { mode: 'retrofit', pkg, hasExpo: true, hasHono: false })
})

test('retrofit: an expo root dependency alone marks hasExpo (no app.config yet)', () => {
  const d = dir('epah-det-expodep-')
  writePkg(d, { name: 'x', dependencies: { expo: '~54.0.0' } })
  writeFileSync(join(d, 'pnpm-workspace.yaml'), "packages:\n  - 'apps/*'\n")
  const r = detect(d)
  assert.equal(r.mode, 'retrofit')
  assert.equal(r.hasExpo, true)
  assert.equal(r.hasHono, false)
})

test('retrofit: hono root dependency alone → hasHono true, hasExpo false', () => {
  const d = dir('epah-det-honodep-')
  writePkg(d, { name: 'x', dependencies: { hono: '^4.0.0' } })
  writeFileSync(join(d, 'pnpm-workspace.yaml'), "packages:\n  - 'apps/*'\n")
  const r = detect(d)
  assert.equal(r.mode, 'retrofit')
...
test('retrofit: both markers present → hasExpo and hasHono both true', () => {
  const d = dir('epah-det-both-')
